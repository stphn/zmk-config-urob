...
draw:
    #!/usr/bin/env bash
    set -euo pipefail
    nix develop --command bash -c 'keymap -c "{{ draw }}/config.yaml" parse -z "{{ config }}/base.keymap" --virtual-layers Combos >"{{ draw }}/base.yaml"'
    yq -Yi '.combos.[].l = ["Combos"]' "{{ draw }}/base.yaml"
    nix develop --command bash -c 'keymap -c "{{ draw }}/config.yaml" draw "{{ draw }}/base.yaml" -k "ferris/sweep" >"{{ draw }}/base.svg"'
